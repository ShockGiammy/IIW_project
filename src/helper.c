...
int SendFile(int socket_desc, char* file_name, char* response) {
	...
	while(sender_wind.tot_acked < file_size ) {
		printf("%d bytes left out of %d\n", sender_wind.tot_acked, file_size);
		...
		if(sender_wind.on_the_fly == sender_wind.max_size || n_read == file_size) {
			if(recv_tcp(socket_desc, buffer, 37) > 0) { //we expect a buffer with only header and no data
				...
				if(recv_segm.ack_number == sender_wind.next_to_ack) {
					...
					for(k = 0; k < sender_wind.n_seg; k++) {
						...
						memset(send_segm[(i+k)%6].data, 0, sizeof(char)*(strlen(send_segm[(i+k)%6].data)));
					}
					printf("\n");
				}
			}
			...
			memset(recv_segm.data, 0, sizeof(char)*(strlen(recv_segm.data)));
			memset(response, 0, sizeof(char)*(strlen(response)));
		}
	}
...
int RetrieveFile(int socket_desc, char* fname) {
	...
	while ((n = recv_tcp(socket_desc, retrieveBuffer, MSS+75)) > 0) {
				...
				if(n = recv_tcp(socket_desc, retrieveBuffer, MSS+75) > 0) {
					...
					if(strcmp(second_segm->data, "END") == 0) {
						if(list_length != 0) {
							...
							ack_segments(fd,socket_desc, &list_length, &buf_segm, &ack,  &recv_win);
						}
						else {
							printf("file receiving completed \n");
							fflush(stdout);
